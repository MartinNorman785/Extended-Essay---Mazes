    ...
    def make_random_walls(self, n):
        for x in range(n):
            if random.choice([True, True, True, False]): # Vertical Barrier
                random_gap = min(max(int(random.gauss(mu=self.HEIGHT / 5, sigma=self.HEIGHT / 5)), 1), self.HEIGHT // 5)
                self.make_random_wall(random_gap, random.sample(range(self.LENGTH), 1)[0], True)
            else:
                random_gap = min(max(int(random.gauss(mu=self.LENGTH / 5, sigma=self.LENGTH / 5)), 1), self.LENGTH // 5)
